...
describe('readlineAsync test suite', () => {
  ...
  describe('questionGroup method', () => {
    it('should return the answers when all of them are valid and restart from the first question when not', async () => {
      ...
      expect(mockQuestion.mock.calls[1][0]).toMatch(firstFakeQuestionValidAnswer)
      expect(mockQuestion.mock.calls[3][0]).toMatch(firstFakeQuestionValidAnswer)
      expect(mockQuestion.mock.calls[4][0]).toMatch(questions[1].validAnswers[0])
      expect(mockQuestion.mock.calls[6][0]).toMatch(firstFakeQuestionValidAnswer)
      expect(mockQuestion.mock.calls[7][0]).toMatch(questions[1].validAnswers[0])
      expect(mockQuestion.mock.calls[10][0]).toMatch(firstFakeQuestionValidAnswer)
      expect(mockQuestion.mock.calls[11][0]).toMatch(questions[1].validAnswers[0])
    })
  })
})
